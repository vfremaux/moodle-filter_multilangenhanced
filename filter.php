...
function multilangenhanced_filter($courseid, $text) {
    global $CFG;

    if (isset($CFG->multilangenhanced_target_language)){
    	$mylang = str_replace('_utf8', '', $CFG->multilangenhanced_target_language);
    } else {
    ...
    if (!array_key_exists($mylang, $parentcache)) {
        ...
        $parentcache[$mylang] = str_replace('_utf8', '', $parentlang);
    } else {
        $parentlang = str_replace('_utf8', '', $parentcache[$mylang]);
    }
...
function multilang_filter_lang_impl($langblock) {
	...
	// PATCH : Use forced override for customlabels
    if (isset($CFG->multilangenhanced_target_language)){
    	$mylang = str_replace('_utf8', '', $CFG->multilangenhanced_target_language);
    }
	// PATCH
